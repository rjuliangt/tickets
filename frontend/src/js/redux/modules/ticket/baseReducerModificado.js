...
export const createReducer = (
    ...
    const listar = (page = 1) => (dispatch, getStore) => {
        ...
        let formData = {}
        if ( getStore().form.formularioFiltro ) {
            // formData = _.cloneDeep(getStore().form.formularioFiltro.values);
            formData = getStore().form.formularioFiltro.values;
        }
        ...
        const params = { page, ...parametro};
        params.ordering = resource[storeId].ordering;
        params.search = resource[storeId].search;
    ...
    const leer = (id) => (dispatch) => {
        ...
        api.get(`${endpoint}/${id}`)
            .then((response) => {
                console.log("leer base", response);
